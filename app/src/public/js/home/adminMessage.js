"use strict"
 ...
 var meageKindes =""
 var  extre = ""
 var meageKindesCheck =""
//checkbox value 값 추출 및 중복 클릭 방지 함수
function typechk(){
  ...
  checkBoxes.forEach((ele, index)=>
    ...
    ele.addEventListener('change', function (e) {
      ...
      for (let i = 0; i < checkBoxes.length; i++){
          ...
          if(checkBoxes[i].checked && checkBoxes[i] !== e.currentTarget){
            checkBoxes[i].checked = !checkBoxes[i].checked
            
          }
     
    }
     meageKindesCheck =index

    console.log(meageKindesCheck)
    checkBoxes[index].value //click
    meageKindes =  e.currentTarget
        
        if(extre !== meageKindes ) {
          console.log("1진입")
          
          extre = meageKindes
          return meageKindes
       
      } else if(extre === meageKindes) {
        console.log("2진입")
        extre =""
        meageKindes=""
        return
...
const labal1 = document.createElement("labal")
  ...
  td2.appendChild(div2) , div2.className =" p-2 mt-2 text-black" ,div2.innerText = data[i].messageTitl +` - (${data[i].sendeType})`
...
function K_A_S_B(messageTitl, meageContents) {
  ...
  if(!DayDeadline && !TimeDeadline) {return alert("알자 및 시간 선택은 하나 이상 해주세요")}
   if(!messageTitl || !meageContents) {return alert("메세지 제목 및 내용이 존재합니다.")}
    if(!meageKindes) {return alert("고정석 / 자유석 / 기간제 에 체크해주세요")}
    
  const req = {
    messageKindeindexOf : meageKindesCheck,
    sendeType:meageKindes,    
    messageTitl,
    meageContents,
